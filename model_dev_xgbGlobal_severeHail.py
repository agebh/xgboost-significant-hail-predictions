...
def main():
    ...
    # region contribution scaling
    target_us_share = 1.0
    target_eu_share = 1.0 # no target share for severe hail prediction

    run_date = "xgbGlobal_severeHail/final_dall"
    ...
    def compute_targeted_weights_eu(y, months):
        weights = np.ones(len(y), dtype=np.float32)
    
        pos = (y == 1)
    
        weights[pos & (months == 10)] *= 0.3   # penalize October
        return weights
    ...
    def save_shap_global_and_dependence(
        booster, 
        X_explain, 
        feature_names, 
        plots_dir, 
    ...
    for ii, (tr_idx, val_idx, te_idx) in enumerate(zip(fold_train, fold_val, fold_test)):
        ...
        y_hb_all_eu = np.array(hail_bin_eu[all_idx, :, :]).astype(float)
        y_sz_all_eu = np.array(hail_size_eu[all_idx, :, :]).astype(float)
        ...
        y_hb_all_eu[:, lsm_eu < 0.5] = np.nan
        y_sz_all_eu[:, lsm_eu < 0.5] = np.nan
        ...
        land_mask_all_eu, year_flat_all_eu, month_flat_all_eu, hb_flat_all_eu, sz_flat_all_eu, valid_binary_all_eu, y_all_eu = \
            prepare_binary_obs(y_hb_all_eu, y_sz_all_eu, dates_all, threshold_cm)
        ...
        idx_keep_all_eu, _, _ = downsample_negatives(y_all_eu, ds_factor_eu, np.random.default_rng(9100 + ii))
        ...
        y_all_bal_eu = y_all_eu[idx_keep_all_eu]
        ...
        X_grid_all_eu = np.copy(rgr_era_varall_eu[all_idx, :, :, :]).astype(np.float32)
        ...
        X_grid_all_eu[:, :, lsm_eu < 0.5] = np.nan
        ...
        X_all_eu_bin = flatten_predictors(X_grid_all_eu, land_mask_all_eu, valid_binary_all_eu)
        ...
        X_all_bal_eu = X_all_eu_bin[idx_keep_all_eu, :] 
        ...
        month_all_bal_eu = month_flat_all_eu[idx_keep_all_eu]
        ...
        rng_pos_eu_tr = np.random.default_rng(3000 + ii)
        rng_pos_eu_train = np.random.default_rng(4000 + ii)
        
        pos_keep_frac_us = {4: 0.35} 
        pos_keep_frac_eu = {10: 0.25}
        ...
        idx_keep_pos_tr_eu = downsample_positive_months(
            y_tr_bal_eu, month_tr_bal_eu, pos_keep_frac_eu, rng_pos_eu_tr
        )
        idx_keep_pos_train_eu = downsample_positive_months(
            y_train_bal_eu, month_train_bal_eu, pos_keep_frac_eu, rng_pos_eu_train
        )
        idx_keep_pos_all_eu = downsample_positive_months(
            y_all_bal_eu, month_all_bal_eu, pos_keep_frac_eu, np.random.default_rng(9300 + ii)
        )
        ...
        # X_tr_bal_eu = X_tr_bal_eu[idx_keep_pos_tr_eu, :]
        # y_tr_bal_eu = y_tr_bal_eu[idx_keep_pos_tr_eu]
        # month_tr_bal_eu = month_tr_bal_eu[idx_keep_pos_tr_eu]

        # X_train_bal_eu = X_train_bal_eu[idx_keep_pos_train_eu, :]
        # y_train_bal_eu = y_train_bal_eu[idx_keep_pos_train_eu]
        # month_train_bal_eu = month_train_bal_eu[idx_keep_pos_train_eu]

        # X_all_bal_eu = X_all_bal_eu[idx_keep_pos_all_eu, :]
        # y_all_bal_eu = y_all_bal_eu[idx_keep_pos_all_eu]
        # month_all_bal_eu = month_all_bal_eu[idx_keep_pos_all_eu]
        ...
        X_all_bal = np.concatenate([X_all_bal_us, X_all_bal_eu], axis=0)
        y_all_bal = np.concatenate([y_all_bal_us, y_all_bal_eu], axis=0)
        ...
        sw_tr_eu = compute_targeted_weights_eu(y_tr_bal_eu, month_tr_bal_eu)
        sw_train_eu = compute_targeted_weights_eu(y_train_bal_eu, month_train_bal_eu)
        sw_all_eu = compute_targeted_weights_eu(y_all_bal_eu, month_all_bal_eu)
        ...
        # ------------------------------------------------------------
        # DMatrices with sample weights (no sample weights)
        # ------------------------------------------------------------
        ...
        spw_tr_damped = 0.01*spw_tr            #sqrt(spw_tr), 0.3*spw_tr 
        spw_train_damped = 0.01*spw_train       #sqrt(spw_train), 0.3*spw_train
        ...
        # ------------------------------------------------------------
        # SHAP for final model trained on all available data
        # only for fold 0
        # ------------------------------------------------------------
        if ii == 0:
            final_shap_plots_dir = plots_dir / "final_model_shap"
            final_shap_stats_dir = stats_dir / "final_model_shap"

            final_shap_plots_dir.mkdir(parents=True, exist_ok=True)
            final_shap_stats_dir.mkdir(parents=True, exist_ok=True)
            ...
            try:
                save_shap_global_and_dependence(
                    ...
                    X_explain=X_all_bal,
                    feature_names=feature_names,
                    plots_dir= plots_dir / str(final_shap_plots_dir),
                    stats_dir= stats_dir / str(final_shap_stats_dir),
                    fold=0,
                    rng_seed=999,
                    max_samples=50000,
                    topk_dependence=8,
                ...
                print(f"[SHAP FINAL] Failed for final model fold 0: {exc}")
                traceback.print_exc()
        ...
        # ------------------------------------------------------------
        # SHAP (SHAP for every fold, not final model)
        # ------------------------------------------------------------
        # try:
        #     save_shap_global_and_dependence(
        ...
        # Save fold outputs
        # ------------------------------------------------------------
        model_path = os.path.join(models_dir, f"xgbGlobal_severeHail_fold{ii}.json")
        model_path_dall = os.path.join(models_dir, f"xgbGlobal_sigHail_FINAL_2000-2022_fold{ii}.json")
